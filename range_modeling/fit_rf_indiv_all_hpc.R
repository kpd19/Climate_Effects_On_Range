...
forest_current <- read_csv("data/forest_species_pa.csv")
habitat_features <- read_csv("data/all_habitat_features.csv")
...
# lag only temperature, only precipitation, only relative humidity
# lag 2/3
# different lags
# split the weather data apart and remerge it
...
write_csv(stats1, paste0("rf_output/stats_update_lag",lag_temp,lag_pr,lag_rh,".csv"))
write_csv(stats2, paste0("rf_output/nums_update_lag",lag_temp,lag_pr,lag_rh,".csv"))
write_csv(rocl0, paste0("rf_output/roc_update_lag",lag_temp,lag_pr,lag_rh,".csv"))
write_csv(rfl0, paste0("rf_output/predictions_update_lag",lag_temp,lag_pr,lag_rh,".csv"))
...
#   
#   temp <- get_conf_int(rf_model, rf_train_lag0,imp_vars$variables[i],imp_vars$type[i], mod = 'rf')
...
write_csv(all_pds, paste0("rf_output/partial_dependence_update_lag",lag_temp,lag_pr,lag_rh,".csv"))
