...
for(i in 1:6){
  travel_time <- raster(paste0("/Volumes/My Book/Synchrony/spatial/travel_time_to_cities_",i,".tif"))
  ...
  write_csv(sp_pop_long,paste0('data/travel_time_',i,'.csv'))
}
...
plt_min1 <- tt %>% drop_na(number) %>% #filter(size != '50,000 to 100,000 inhabitants') %>% 
...
grid.arrange(plt_min1,plt_min2)
dev.off()

min_travel <- tt %>% drop_na(number) %>% filter(size != '50,000 to 100,000 inhabitants') %>% 
  group_by(lat,lon,manual_id,source,elev2) %>% summarize(min_time = min(number))
min_travel2 <- tt %>% drop_na(number) %>% #filter(size != '50,000 to 100,000 inhabitants') %>% 
  group_by(lat,lon,manual_id,source,elev2) %>% summarize(min_time = min(number))

write_csv(min_travel, 'data/min_travel_time_1-5.csv')
write_csv(min_travel2, 'data/min_travel_time_1-6.csv')
